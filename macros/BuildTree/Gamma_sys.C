...
/// PicoDst headers
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoDstReader.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoDst.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoEvent.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoTrack.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoBTofHit.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoBTowHit.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoEmcTrigger.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoBTofPidTraits.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StPicoEvent/StPicoTrackCovMatrix.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StEpdUtil/StEpdEpFinder.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StRefMultCorr/StRefMultCorr.h"
#include "/star/data01/pwg/rexwg/Isobar/StRoot/StRefMultCorr/CentralityMaker.h"
...
const int opt_SP = 1;  //1 means scalar product; 0 means event plane method
//const int opt_sys = 2; //2 means inclusive
const int opt_noPileup = 1; //1 means applying cuts to reject pile-up events
const int opt_wt_EP = 1; //1 means use pT as weight for EP reconstruction
const int opt_eff = 0; //0 means eff = 1 everywhere
...
const float Eta_noMerge = 0.05; //remove track merging
const float Eta_noHBT = 0.2;	//remove HBT, Coulomb and electrcon pairs
...
const float EtaCut_asso = 0.99999;
const float DcaCut_asso = 3.0;         //2
const int nFitHits_asso = 15;	//15, number of fit hits

const int opt_tof = 0;
const float pt_trig_up = 2.0;
const float pt_trig_lo = 0.2;
const float EtaCut_trig = 0.99999;
const float DcaCut_trig = 3.0;
const int nFitHits_trig = 15;  //15
const int nFitRatio_trig_lo = 0.52;  //0.52
const int nFitRatio_trig_up = 1.05;  //1.05

const Float_t Vz_cut_lo = -35;    	//30, Vz
const Float_t Vz_cut_up = 25;
const Float_t Vr_cut = 2;	//2, Vr
const float Vz_diff = 5;        //5, difference between Vz{TPC} and Vz{VPD}
const int run_sta = 71000;
const int run_end = 168000;
const int cenDef[9] = {8, 15, 27, 44, 68, 102, 149, 213, 255};  //isobar

//const int run_num = 36;
//const int RunList_A[run_num] = {19084026,19084069,19084129,19084161,19084224,19084260,19084280,19084304,19084339,19085017,
//19085037,19085095,19085126,19085140,19085187,19085229,19085258,19085318,19086041,19086049,
//19086080,19086088,19086120,19086206,19086274,19086307,19087209,19088247,19089339,19094190,
//19096010,19097021,19097277,19098055,19098275,19098316};
//const int RunList_B[run_num] = {19084006,19084038,	19084100,19084160,19084193,19084253,19084272,19084292,19084312,19085005,	
//19085029,19085073,19085105,19085136,19085168,19085197,19085252,19085290,19086008,19086048,	
//19086073,19086081,19086109,19086141,19086242,19086299,19086339,19087240,19089020,19092141,	
//19096009,19096126,19097104,19098020,19098158,19098305};
...
//defining histograms
	...
	TH2D *Ref_TOF = new TH2D("Ref_TOF","Ref_TOF",500,0.5,500.5,5000,0.5,5000.5);
        ...
        TProfile *TPC_Day3_cos1 = new TProfile("TPC_Day3_cos1","cos(psi) vs Run", run_end-run_sta,run_sta,run_end,-1,1);
        TProfile *TPC_Day3_sin1 = new TProfile("TPC_Day3_sin1","sin(psi) vs Run", run_end-run_sta,run_sta,run_end,-1,1);
	...
	TH1D *hTally = new TH1D("hTally","hTally",12,-0.5,11.5);
	...
	TH1D *Hist_nHits = new TH1D("Hist_nHits","Hist_nHits",100,0,100);
        TH1D *Hist_HitRatio = new TH1D("Hist_HitRatio","Hist_HitRatio",150,0,1.5);
	...
	TH2D* wt = new TH2D("Order1etaWeight","Order1etaWeight",100,1.5,6.5,9,0,9);
        ...
        TProfile *Hist_v2_eta_obs4 = new TProfile("Hist_v2_eta_obs4","Hist_v2_eta_obs4",40,-1,1,-100,100,"");
	...
	TProfile *pV22_int_obs1 = new TProfile("pV22_int_obs1","pV22_int_obs1", 4,0.5,4.5,-100,100,"");
        TProfile *pV22_int_obs3 = new TProfile("pV22_int_obs3","pV22_int_obs3", 4,0.5,4.5,-100,100,"");
        TProfile2D *pV22_Deta_obs3 = new TProfile2D("pV22_Deta_obs3","pV22_Deta_obs3", 4,0.5,4.5, 40,0,2,-100,100,"");
        ...
        TProfile *pParity_noMerge_ss_obs1   = new TProfile("Parity_noMerge_ss_obs1","Parity_noMerge_ss_obs1",4,0.5,4.5,-100,100,"");
        TProfile *pParity_noMerge_ss_obs3   = new TProfile("Parity_noMerge_ss_obs3","Parity_noMerge_ss_obs3",4,0.5,4.5,-100,100,"");
        ...
        TProfile *pDelta_int_ss_obs1 = new TProfile("Delta_int_ss_obs1","Delta_int_ss_obs1",7,0.5,7.5,-100,100,"");
        TProfile *pDelta_int_ss_obs3 = new TProfile("Delta_int_ss_obs3","Delta_int_ss_obs3",7,0.5,7.5,-100,100,"");
        ...
        TProfile *pDelta_noMerge_ss_obs1   = new TProfile("Delta_noMerge_ss_obs1","Delta_noMerge_ss_obs1",4,0.5,4.5,-100,100,"");
        TProfile *pDelta_noMerge_ss_obs3   = new TProfile("Delta_noMerge_ss_obs3","Delta_noMerge_ss_obs3",4,0.5,4.5,-100,100,"");
        ...
        TProfile *pTemp_v2_noMerge= new TProfile("pTemp_v2_noMerge","pTemp_v2_noMerge",6,0.5,6.5,-100,100,"");
        TProfile *pTemp_v2_noHBT = new TProfile("pTemp_v2_noHBT","pTemp_v2_noHBT",6,0.5,6.5,-100,100,"");
        TProfile *pTemp_parity_e_noMerge = new TProfile("pTemp_parity_e_noMerge","pTemp_parity_e_noMerge",8,0.5,8.5,-100,100,"");
        TProfile *pTemp_parity_e_noHBT = new TProfile("pTemp_parity_e_noHBT","pTemp_parity_e_noHBT",8,0.5,8.5,-100,100,"");
        TProfile *pTemp_parity_w_noMerge = new TProfile("pTemp_parity_w_noMerge","pTemp_parity_w_noMerge",8,0.5,8.5,-100,100,"");
        TProfile *pTemp_parity_w_noHBT = new TProfile("pTemp_parity_w_noHBT","pTemp_parity_w_noHBT",8,0.5,8.5,-100,100,"");
        TProfile *pTemp_delta_noMerge = new TProfile("pTemp_delta_noMerge","pTemp_delta_noMerge",8,0.5,8.5,-100,100,"");
	...
	TH1D* Hist_Q  = new TH1D("Hist_Q","Hist_Q",250,0,5);
        ...
        TProfile *p_v2e_noMerge_Q2_obs2 = new TProfile("p_v2e_noMerge_Q2_obs2","p_v2e_noMerge_Q2_obs2",250,0,25,-100,100,"");
        TProfile *p_v2w_noMerge_Q2_obs2 = new TProfile("p_v2w_noMerge_Q2_obs2","p_v2w_noMerge_Q2_obs2",250,0,25,-100,100,"");
        TProfile *p_v2e_noMerge_Q2_P_obs2 = new TProfile("p_v2e_noMerge_Q2_P_obs2","p_v2e_noMerge_Q2_P_obs2",250,0,25,-100,100,"");
        TProfile *p_v2w_noMerge_Q2_P_obs2 = new TProfile("p_v2w_noMerge_Q2_P_obs2","p_v2w_noMerge_Q2_P_obs2",250,0,25,-100,100,"");
        TProfile *p_v2e_noMerge_Q2_N_obs2 = new TProfile("p_v2e_noMerge_Q2_N_obs2","p_v2e_noMerge_Q2_N_obs2",250,0,25,-100,100,"");
        TProfile *p_v2w_noMerge_Q2_N_obs2 = new TProfile("p_v2w_noMerge_Q2_N_obs2","p_v2w_noMerge_Q2_N_obs2",250,0,25,-100,100,"");
        TProfile2D *pParity_e_noMerge_Q2_obs2 = new TProfile2D("Parity_e_noMerge_Q2_obs2","Parity_e_noMerge_Q2_obs2",4,0.5,4.5,250, 0, 25, -100,100,"");
        TProfile2D *pParity_w_noMerge_Q2_obs2 = new TProfile2D("Parity_w_noMerge_Q2_obs2","Parity_w_noMerge_Q2_obs2",4,0.5,4.5,250, 0, 25, -100,100,"");
        TProfile2D *pDelta_noMerge_Q2_obs2 = new TProfile2D("pDelta_noMerge_Q2_obs2","pDelta_noMerge_Q2_obs2",4,0.5,4.5,250, 0, 25, -100,100,"");
...
TH1D *Read_Hist_Pt = 0, *Read_Hist_Pt_TOF = 0;
...
float Rsin = 0, Rcos = 0;
int iCh = 0;
float correlator0 = 0, correlator2 = 0, correlator3 = 0, correlator4 = 0, correlator4e = 0, correlator4w = 0;

//sub-functions
void DefineHistogram();						//define histograms 
void WriteHistogram(int c, int o, int j);  			//into result ROOT file
...
void Gamma_sys(int cen=1, int opt_weight =1, int job = 1, const Char_t *inFile = "test.list"){	//main_function
	delete gRandom;
	gRandom = new TRandom3(0);

	DefineHistogram();
        char fname_old[200], fname_new[200];
        sprintf(fname_new,"cen%d.weight_1%d%d_job%d.root",cen,nHar-1,nHar,job);
	...
	StEpdEpFinder* mEpFinder = new StEpdEpFinder(9,fname_new,fname_old);
  	...
  	mEpFinder->SetMaxTileWeight(5.0);     	// recommended by EPD group
...
//        mEpFinder->SetEtaWeights(2,wt2);	// eta weight for 2nd-order EP, select different eta range
	...
	for(int i = 0; i < nentries; i++){
		...
		Day2    = (int)((Run%1000000)/10);
                ...
                hTally->Fill(0);
                if(!event->isTrigger(600001) && !event->isTrigger(600011) && !event->isTrigger(600021) && !event->isTrigger(600031)) continue;

		if(!IsGoodEvent(cen)) continue;

                double NBTOFMult_fit =0;
                if(Ntofmatch < 14){
                        NBTOFMult_fit = 0.59207 + 2.1317*Ntofmatch;
                } else{
                        NBTOFMult_fit = 18.770 + 1.0699*Ntofmatch;
                }
//                if(opt_noPileup==1 && RefMult >= NBTOFMult_fit) continue;

                refmultCorrUtil->init(Run);
                if ( refmultCorrUtil->isBadRun(Run) ) continue;
		hTally->Fill(7);
                refmultCorrUtil->initEvent(RefMult, pVz); //, ZDCcoin) ;
                if(opt_noPileup==1 && !refmultCorrUtil->passnTofMatchRefmultCut(1.*RefMult, 1.*Ntofmatch)) continue;
		hTally->Fill(8);
...
//BBC EP
          	if(opt_useEPD==2 && !IsGoodBBC(event)) continue;
	  	MakeBBC_EP(event);
...
if(opt_useEPD == 1 && EPD_EP_east == EPD_EP_west) continue;
...
//ZDC EP
	  	if(opt_useEPD == 3 && !IsGoodZDC(event)) continue;
          	MakeZDC_EP(event);
//flatten EPs
 	  	ShiftPsi();
	  	FillEP_resolution();
...
//////////Real analysis begins here//////////////////////////////////////////////////////////////////////////////////////
		...
		for(int trki = 0; trki < NPTracks; trki++){
			...
			if(opt_eff==0) eff = 1;
                        ...
                        if(opt_tof==1) eff *= eff_tof;

			if(!IsGoodPOI(picoTrack)) continue;
                        if(DCAGlobal<1 && fabs(Eta)<0.9) {      // && Pt>0.2 && Pt*cosh(Eta)<2) {
                                Hist_Pt->Fill(Pt,Eweight);
                                if(picoTrack->isTofTrack()) Hist_Pt_TOF->Fill(Pt,Eweight);
                        }
//			if(!IsGoodPion(dst,picoTrack,1)) continue;
			...
			float wt_EP = 1;
			if(opt_wt_EP==1) wt_EP = Pt;
                        float mQx_i = mQx, mQy_i = mQy;
                        if(IsGoodAsso(picoTrack)) {
                                mQx_i -= wt_EP*cos(PhiAsso_new[trki] * nHar);
                                mQy_i -= wt_EP*sin(PhiAsso_new[trki] * nHar);
                        }
			...
			Hist_TPC_EP_full_m1_flat->Fill(psi_F_new);
                        ...
                        if(opt_SP==1) v2  = cos(nHar*Phi_new[trki])*mQx_i + sin(nHar*Phi_new[trki])*mQy_i;
			...
			for(int trkj = trki+1; trkj < NPTracks; trkj++) {
				...
				if(opt_eff==0) eff2 = 1;
                                ...
                                if(opt_tof==1) eff2 *= eff_tof2;

//				if(!IsGoodPion(dst,picoTrack2,1)) continue;
                                hDpt->Fill(fabs(Pt-Pt2),Eweight);

                        	float wt_EP2 = 1;
                        	if(opt_wt_EP==1) wt_EP2 = Pt2;
				float mQx_j = mQx_i, mQy_j = mQy_i;
				if(IsGoodAsso(picoTrack2)) {			
                        		mQx_j -= wt_EP2*cos(PhiAsso_new[trkj] * nHar);
                        		mQy_j -= wt_EP2*sin(PhiAsso_new[trkj] * nHar);
				}
				...
				correlator2 = cos(2*Phi1_new - 2*Phi2_new)*100;
                                ...
                                if(opt_SP ==1) correlator4 = cos(Phi1_new + (nHar-1)*Phi2_new)*mQx_j + sin(Phi1_new + (nHar-1)*Phi2_new)*mQy_j;
				if(opt_HighOrder==1) {
                              		correlator0 = cos(Phi - (nHar+1)*Phi2 + nHar*psi_F2_new)*100;
                              		correlator4e= cos(Phi1_new - (nHar+1)*Phi2_new + nHar*TPC_EP_for_new)*100;
                              		correlator4w= cos(Phi1_new - (nHar+1)*Phi2_new + nHar*TPC_EP_bac_new)*100;
				...
				if(opt_useEPD==1 || opt_useEPD==2) correlator4 = 0.5*(correlator4e+correlator4w);
			        ...
			        pV22_int_obs1->Fill(1,correlator2);
        			pV22_int_obs3->Fill(1,correlator2,Eweight/eff/eff2);

				//no track merging
                                if(fabs(Eta-Eta2)> Eta_noMerge) {
                                        pV22_int_obs1->Fill(2,correlator2);
                                        pV22_int_obs3->Fill(2,correlator2,Eweight/eff/eff2);
					Hist_v2_eta_obs3->Fill(Eta,v2,Eweight/eff);
                                        if((opt_useEPD==0 && fabs(Eta)<Eta_ESE_Cut && fabs(Eta2)<Eta_ESE_Cut) || opt_useEPD>0) {
                                                pTemp_v2_noMerge->Fill(1,v2e, 1./eff);pTemp_v2_noMerge->Fill(2,v2w, 1./eff);
                                                if(Charge>0) {pTemp_v2_noMerge->Fill(3,v2e, 1./eff); pTemp_v2_noMerge->Fill(4,v2w, 1./eff);}
                                                if(Charge<0) {pTemp_v2_noMerge->Fill(5,v2e, 1./eff); pTemp_v2_noMerge->Fill(6,v2w, 1./eff);}
					}
                                }
				//no HBT
				if(fabs(Eta-Eta2)> Eta_noHBT) {
					pV22_int_obs1->Fill(3,correlator2);
                                	pV22_int_obs3->Fill(3,correlator2,Eweight/eff/eff2);
                                        Hist_v2_eta_obs4->Fill(Eta,v2,Eweight/eff);
                                        if((opt_useEPD==0 && fabs(Eta)<Eta_ESE_Cut && fabs(Eta2)<Eta_ESE_Cut) || opt_useEPD>0) {
	...
	WriteHistogram(cen,opt_weight,job);
//	if(opt_weight==1) {
		mEpFinder->Finish();
...
hTally->Fill(3);
        ...
        hVertexZ->Fill(pVz);
hTally->Fill(4);
        if((pVz > Vz_cut_up) || (pVz < Vz_cut_lo)) return false;        
...
hTally->Fill(5);
//	if(opt_sys==0 && find(RunList_A,RunList_A+run_num,Run)==(RunList_A+run_num)) return false;
//	if(opt_sys==1 && find(RunList_B,RunList_B+run_num,Run)==(RunList_B+run_num)) return false;
hTally->Fill(6);	
	return true;
}
//////////////////////////////////////////////
bool CountCharge(StPicoDst *d) {
		...
		int nHF = picoTrack->nHitsFit();
        	int nHM = picoTrack->nHitsMax();
		Hist_nHits->Fill(nHF);
		if(nHF>=nFitHits_trig) Hist_HitRatio->Fill(nHF/float(nHM));
...
void MakeTPC_EP(StPicoDst *d, int *iTr) {
          ...
          TVector2 mQ, mQ0, mQ1, mQ2, mQ3, mQ4;
          mQx=0., mQy=0.;
          float mQQx=0., mQQy=0., mQx0=0.,mQy0=0.,mQx1=0., mQy1=0., mQx2=0., mQy2=0., mQx3=0., mQy3=0., mQx4=0., mQy4=0.;
          Fcount = 0, POIcount = 0;
          int Qcount = 0;
		...
		float wt_EP = 1;
		if(opt_wt_EP==1) wt_EP = PtAsso;
                mQx += wt_EP*cos(PhiAsso_new[trk]*nHar); mQy += wt_EP*sin(PhiAsso_new[trk] * nHar);
		mQx0+= wt_EP*cos(PhiAsso_new[trk]);	mQy0 += wt_EP*sin(PhiAsso_new[trk]);
                if(fabs(EtaAsso)<Eta_ESE_Cut && IsGoodPOI(picoTrack)) {mQQx+= wt_EP*cos(PhiAsso_new[trk]*nHar); mQQy += wt_EP*sin(PhiAsso_new[trk] * nHar);Qcount++;}
                if(iTr[Fcount] > Scount) {mQx1 += wt_EP*cos(PhiAsso_new[trk]*nHar); mQy1 += wt_EP*sin(PhiAsso_new[trk] * nHar);}
                else {mQx2 += wt_EP*cos(PhiAsso_new[trk] * nHar); mQy2 += wt_EP*sin(PhiAsso_new[trk] * nHar);}
                if(EtaAsso> Eta_EP_Cut) {mQx3 += wt_EP*cos(PhiAsso_new[trk]*nHar); mQy3 += wt_EP*sin(PhiAsso_new[trk] * nHar);}
                if(EtaAsso<-Eta_EP_Cut) {mQx4 += wt_EP*cos(PhiAsso_new[trk]*nHar); mQy4 += wt_EP*sin(PhiAsso_new[trk] * nHar);}
                Fcount++;
          }
          mQ.Set(mQx, mQy); mQ0.Set(mQx0, mQy0);
	  mQ1.Set(mQx1, mQy1); mQ2.Set(mQx2, mQy2); mQ3.Set(mQx3, mQy3); mQ4.Set(mQx4, mQy4);
          ...
          TPC_Day3_cos1->Fill(Day3, cos(mQ0.Phi()));
          TPC_Day3_sin1->Fill(Day3, sin(mQ0.Phi()));
...
void FillEP_resolution() {
          ...
          Hist_cos->Fill(3,cos(nHar*TPC_EP_for_new -nHar*TPC_EP_full_new), Eweight);
          Hist_cos->Fill(4,cos(nHar*TPC_EP_bac_new -nHar*TPC_EP_full_new), Eweight);
	  ...
	  if(opt_useEPD==2) cos_fb = cos(nHar*BBC_EP_east_new-nHar*BBC_EP_west_new);
          ...
          p_RefMult_Q2->Fill(Q2,Fcount, Eweight);
          Hist_RefMult_Q2->Fill(Q2,Fcount, Eweight);

          if(net_Nch_Asym>-99) {
                p_netChAsym_cos->Fill(net_Nch_Asym,cos_fb, Eweight);
...
bool IsGoodPOI(StPicoTrack *p) {
	if(opt_tof==1 && !(p->isTofTrack())) return false;
	int nHF = p->nHitsFit();
...
/////////////////////////////////
void WriteHistogram(int c, int o, int j) {
	char fname_out[200];
	sprintf(fname_out,"cen%d.gamma1%d%d_EP%d_ptWeight%d_Boost%d_SP%d_job%d.root",c,nHar-1,nHar,opt_useEPD,opt_wt_EP,opt_boost,opt_SP,j);
	if(opt_HighOrder==1) sprintf(fname_out,"cen%d.gamma1%d%d_EP%d_ptWeight%d_Boost%d_SP%d_job%d.root",c,nHar+1,nHar,opt_useEPD,opt_wt_EP,opt_boost,opt_SP,j);
	if(o==1) sprintf(fname_out,"cen%d.v%d_SP%d_job%d.root",c,nHar,opt_SP,j);
...
int ReadWeight(char* InFileName) {
		...
		Read_Hist_Pt = (TH1D*)fWgt->Get("Hist_Pt");
		Read_Hist_Pt_TOF = (TH1D*)fWgt->Get("Hist_Pt_TOF");
		TOF_eff = (TH1D*)Read_Hist_Pt_TOF->Clone();
        	TOF_eff->Divide(Read_Hist_Pt);
...
/////////////////////////////////////////////////////
void Fillv2() {
			pV22_int_obs1->Fill(4,v2);
			pV22_int_obs3->Fill(4,v2,Eweight/eff);
...
void FillGamma(int ord) {
	pV22_Deta_obs3->Fill(ord, fabs(Eta-Eta2),correlator2,Eweight/eff/eff2);
	if(ord==4) {
	...
	if((opt_useEPD==0 && fabs(Eta)<0.5 && fabs(Eta2)<0.5) || opt_useEPD>0) {
        	...
        	pTemp_delta->Fill(ord,correlator3,1./eff/eff2);
                if(fabs(Eta-Eta2)>Eta_noMerge) {
                        pTemp_parity_e_noMerge->Fill(ord,correlator4e,1./eff/eff2);
                        pTemp_parity_w_noMerge->Fill(ord,correlator4w,1./eff/eff2);
                        pTemp_delta_noMerge->Fill(ord,correlator3,1./eff/eff2);
                }
		if(fabs(Eta-Eta2)>Eta_noHBT) {
        ...
        if(fabs(Eta-Eta2)>Eta_noMerge) {
                pParity_noMerge_ss_obs1->Fill(ord,correlator4);
                pParity_noMerge_ss_obs3->Fill(ord,correlator4,Eweight/eff/eff2);
                pDelta_noMerge_ss_obs1->Fill(ord,correlator3);
                pDelta_noMerge_ss_obs3->Fill(ord,correlator3,Eweight/eff/eff2);
        }
        if(fabs(Eta-Eta2)>Eta_noHBT) {
...
void WrapUpESE() {
        ...
        if(Temp_v2e!=0) {p_v2e_Q2_obs2->Fill(Q2,Temp_v2e, Eweight);}
        if(Temp_v2w!=0) {p_v2w_Q2_obs2->Fill(Q2,Temp_v2w, Eweight);}
        if(Temp_v2e_P!=0) {p_v2e_Q2_P_obs2->Fill(Q2,Temp_v2e_P, Eweight);}
	if(Temp_v2w_P!=0) {p_v2w_Q2_P_obs2->Fill(Q2,Temp_v2w_P, Eweight);}
        if(Temp_v2e_N!=0) {p_v2e_Q2_N_obs2->Fill(Q2,Temp_v2e_N, Eweight);}
        if(Temp_v2w_N!=0) {p_v2w_Q2_N_obs2->Fill(Q2,Temp_v2w_N, Eweight);}
        if(Temp_parity1e!=0) {pParity_e_Q2_obs2->Fill(1,Q2, Temp_parity1e, Eweight);}
        if(Temp_parity2e!=0) {pParity_e_Q2_obs2->Fill(2,Q2, Temp_parity2e, Eweight);}
        if(Temp_parity3e!=0) {pParity_e_Q2_obs2->Fill(3,Q2, Temp_parity3e, Eweight);}
        if(Temp_parity4e!=0) {pParity_e_Q2_obs2->Fill(4,Q2, Temp_parity4e, Eweight);}
        if(Temp_parity1w!=0) {pParity_w_Q2_obs2->Fill(1,Q2, Temp_parity1w, Eweight);}
        if(Temp_parity2w!=0) {pParity_w_Q2_obs2->Fill(2,Q2, Temp_parity2w, Eweight);}
        if(Temp_parity3w!=0) {pParity_w_Q2_obs2->Fill(3,Q2, Temp_parity3w, Eweight);}
        if(Temp_parity4w!=0) {pParity_w_Q2_obs2->Fill(4,Q2, Temp_parity4w, Eweight);}
        if(Temp_delta1!=0) {pDelta_Q2_obs2->Fill(1,Q2, Temp_delta1, Eweight);}
        if(Temp_delta2!=0) {pDelta_Q2_obs2->Fill(2,Q2, Temp_delta2, Eweight);}
        if(Temp_delta3!=0) {pDelta_Q2_obs2->Fill(3,Q2, Temp_delta3, Eweight);}
        if(Temp_delta4!=0) {pDelta_Q2_obs2->Fill(4,Q2, Temp_delta4, Eweight);}

        Temp_v2e = pTemp_v2_noMerge->GetBinContent(1);
        Temp_v2w = pTemp_v2_noMerge->GetBinContent(2);
        Temp_v2e_P = pTemp_v2_noMerge->GetBinContent(3);
        Temp_v2w_P = pTemp_v2_noMerge->GetBinContent(4);
        Temp_v2e_N = pTemp_v2_noMerge->GetBinContent(5);
        Temp_v2w_N = pTemp_v2_noMerge->GetBinContent(6);
        Temp_parity1e = pTemp_parity_e_noMerge->GetBinContent(1);
        Temp_parity2e = pTemp_parity_e_noMerge->GetBinContent(2);
        Temp_parity3e = pTemp_parity_e_noMerge->GetBinContent(3);
        Temp_parity4e = pTemp_parity_e_noMerge->GetBinContent(4);
        Temp_parity1w = pTemp_parity_w_noMerge->GetBinContent(1);
        Temp_parity2w = pTemp_parity_w_noMerge->GetBinContent(2);
        Temp_parity3w = pTemp_parity_w_noMerge->GetBinContent(3);
        Temp_parity4w = pTemp_parity_w_noMerge->GetBinContent(4);
        Temp_delta1 = pTemp_delta_noMerge->GetBinContent(1);
        Temp_delta2 = pTemp_delta_noMerge->GetBinContent(2);
        Temp_delta3 = pTemp_delta_noMerge->GetBinContent(3);
        Temp_delta4 = pTemp_delta_noMerge->GetBinContent(4);

        if(Temp_v2e!=0) {p_v2e_noMerge_Q2_obs2->Fill(Q2,Temp_v2e, Eweight);}
        if(Temp_v2w!=0) {p_v2w_noMerge_Q2_obs2->Fill(Q2,Temp_v2w, Eweight);}
        if(Temp_v2e_P!=0) {p_v2e_noMerge_Q2_P_obs2->Fill(Q2,Temp_v2e_P, Eweight);}
        if(Temp_v2w_P!=0) {p_v2w_noMerge_Q2_P_obs2->Fill(Q2,Temp_v2w_P, Eweight);}
        if(Temp_v2e_N!=0) {p_v2e_noMerge_Q2_N_obs2->Fill(Q2,Temp_v2e_N, Eweight);}
        if(Temp_v2w_N!=0) {p_v2w_noMerge_Q2_N_obs2->Fill(Q2,Temp_v2w_N, Eweight);}
        if(Temp_parity1e!=0) {pParity_e_noMerge_Q2_obs2->Fill(1,Q2, Temp_parity1e, Eweight);}
        if(Temp_parity2e!=0) {pParity_e_noMerge_Q2_obs2->Fill(2,Q2, Temp_parity2e, Eweight);}
        if(Temp_parity3e!=0) {pParity_e_noMerge_Q2_obs2->Fill(3,Q2, Temp_parity3e, Eweight);}
        if(Temp_parity4e!=0) {pParity_e_noMerge_Q2_obs2->Fill(4,Q2, Temp_parity4e, Eweight);}
        if(Temp_parity1w!=0) {pParity_w_noMerge_Q2_obs2->Fill(1,Q2, Temp_parity1w, Eweight);}
        if(Temp_parity2w!=0) {pParity_w_noMerge_Q2_obs2->Fill(2,Q2, Temp_parity2w, Eweight);}
        if(Temp_parity3w!=0) {pParity_w_noMerge_Q2_obs2->Fill(3,Q2, Temp_parity3w, Eweight);}
        if(Temp_parity4w!=0) {pParity_w_noMerge_Q2_obs2->Fill(4,Q2, Temp_parity4w, Eweight);}
        if(Temp_delta1!=0) {pDelta_noMerge_Q2_obs2->Fill(1,Q2, Temp_delta1, Eweight);}
        if(Temp_delta2!=0) {pDelta_noMerge_Q2_obs2->Fill(2,Q2, Temp_delta2, Eweight);}
        if(Temp_delta3!=0) {pDelta_noMerge_Q2_obs2->Fill(3,Q2, Temp_delta3, Eweight);}
        if(Temp_delta4!=0) {pDelta_noMerge_Q2_obs2->Fill(4,Q2, Temp_delta4, Eweight);}
        ...
        if(Temp_v2e!=0) {p_v2e_noHBT_Q2_obs2->Fill(Q2,Temp_v2e, Eweight);}
        if(Temp_v2w!=0) {p_v2w_noHBT_Q2_obs2->Fill(Q2,Temp_v2w, Eweight);}
        if(Temp_v2e_P!=0) {p_v2e_noHBT_Q2_P_obs2->Fill(Q2,Temp_v2e_P, Eweight);}
        if(Temp_v2w_P!=0) {p_v2w_noHBT_Q2_P_obs2->Fill(Q2,Temp_v2w_P, Eweight);}
        if(Temp_v2e_N!=0) {p_v2e_noHBT_Q2_N_obs2->Fill(Q2,Temp_v2e_N, Eweight);}
        if(Temp_v2w_N!=0) {p_v2w_noHBT_Q2_N_obs2->Fill(Q2,Temp_v2w_N, Eweight);}
        if(Temp_parity1e!=0) {pParity_e_noHBT_Q2_obs2->Fill(1,Q2, Temp_parity1e, Eweight);}
        if(Temp_parity2e!=0) {pParity_e_noHBT_Q2_obs2->Fill(2,Q2, Temp_parity2e, Eweight);}
        if(Temp_parity3e!=0) {pParity_e_noHBT_Q2_obs2->Fill(3,Q2, Temp_parity3e, Eweight);}
        if(Temp_parity4e!=0) {pParity_e_noHBT_Q2_obs2->Fill(4,Q2, Temp_parity4e, Eweight);}
        if(Temp_parity1w!=0) {pParity_w_noHBT_Q2_obs2->Fill(1,Q2, Temp_parity1w, Eweight);}
        if(Temp_parity2w!=0) {pParity_w_noHBT_Q2_obs2->Fill(2,Q2, Temp_parity2w, Eweight);}
        if(Temp_parity3w!=0) {pParity_w_noHBT_Q2_obs2->Fill(3,Q2, Temp_parity3w, Eweight);}
        if(Temp_parity4w!=0) {pParity_w_noHBT_Q2_obs2->Fill(4,Q2, Temp_parity4w, Eweight);}
        if(Temp_delta1!=0) {pDelta_noHBT_Q2_obs2->Fill(1,Q2, Temp_delta1, Eweight);}
        if(Temp_delta2!=0) {pDelta_noHBT_Q2_obs2->Fill(2,Q2, Temp_delta2, Eweight);}
        if(Temp_delta3!=0) {pDelta_noHBT_Q2_obs2->Fill(3,Q2, Temp_delta3, Eweight);}
        if(Temp_delta4!=0) {pDelta_noHBT_Q2_obs2->Fill(4,Q2, Temp_delta4, Eweight);}

                pTemp_v2->Reset();
                pTemp_v2_noMerge->Reset();
                pTemp_v2_noHBT->Reset();
                pTemp_parity_e->Reset();
                pTemp_parity_e_noMerge->Reset();
                pTemp_parity_e_noHBT->Reset();
                pTemp_parity_w->Reset();
                pTemp_parity_w_noMerge->Reset();
                pTemp_parity_w_noHBT->Reset();
                pTemp_delta->Reset();
                pTemp_delta_noMerge->Reset();
...
void DefineHistogram() {
	...
	hDpt->Sumw2();
        ...
        while ((key = (TKey*)next())) {
                TString tempStr(key->GetName());
                if (tempStr.Contains("_obs") || tempStr.Contains("_cos")) {
			obj = gROOT->Get(key->GetName());
			((TProfile*) obj)->Sumw2();
		}
        }
