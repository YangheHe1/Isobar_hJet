...
void InitHist(){
    ...
    sprintf(name,"/star/data05/scratch/yanghe/jet_ME_Isobar/M4_R%.1lf_RootFile/%s.root",Jet_R,FileOutput);
...
int main(int argc, char **argv){
    ...
    TChain *mychain = new TChain("METree");
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        double zTpc = mtree.ME_Vz;
        double refMultCorr = mtree.ME_refmult;
        double j_Vx = mtree.ME_Vx;
        double j_Vy = mtree.ME_Vy;
        int _NTracks = mtree.ME_numTrk;
...
//        if(refMultCorr<235&&refMultCorr>54) continue;
        ...
        for(int itrack=0; itrack<_NTracks; itrack++){
            ...
            double _pt = mtree.ME_Pt[itrack];
            double _eta = mtree.ME_Eta[itrack];
            double _phi = mtree.ME_Phi[itrack];
            double _charge = mtree.ME_Charge[itrack];
            double _dca = mtree.ME_Dca[itrack];
