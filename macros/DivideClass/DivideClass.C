...
Float_t mult_start_stop_delta[3] = {255.0,400.0,1.0};
...
int centrality( int _refMult ){
    ...
    float   CentralityBins  [NCENT] = {255,213,149,102,68,44,27,15,8} ;  //
...
void InitHist(){
    ...
    if(Cen_mode==2){
        mult_start_stop_delta[0] = 15.0;
        mult_start_stop_delta[1] = 44.0;
        mult_start_stop_delta[2] = 1.0;
    }
    if(Cen_mode==0){
        mult_start_stop_delta[0] = 0.0;
        mult_start_stop_delta[1] = 400.0;
        mult_start_stop_delta[2] = 1.0;
    }
	...
	for(int i_refM=0;i_refM<stop_refM;i_refM++){
		for(int i_vz=0;i_vz<stop_vz;i_vz++){
			for(int i_psi=0;i_psi<stop_psi;i_psi++){
				...
				TString mixed_event_outfile_name = "/star/data05/scratch/yanghe/mixclass/";
...
int main(int argc, char **argv){
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++){
        ...
        //if(Cen_mode==2){ if(CentID!=6) continue;}
	...
	if(refMultCorr >= NBTOFMult_fit) continue;
        ...
        double totaltrack= pptree.refmult;
