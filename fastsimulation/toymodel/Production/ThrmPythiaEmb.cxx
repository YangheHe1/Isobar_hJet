...
kCen=atoi(gSystem->Getenv("CENTRAL"));
...
name=Form("hnpart_rm_pTl%i",pTl);
  ...
  hJER[pTl]=new TH2D(name, "Jet Energy Resolution;p_{T}^{part};(p_{T}^{dete}-p_{T}^{part})/p_{T}^{part};entries", 100, 0, 100, 200,-1,1);
  ...
  //jet pt vs phi
  name=Form("hntruerm_vs_phi_pTl%i",pTl);
  hntruerm_vs_phi[pTl]=new TH2D(name,"true jet pt(for RM) vs phi;#phi;p_{T,jet}^{part}(GeV/c)",200,0,8,nptbins, ptminbin, ptmaxbin);
  name=Form("hndeterm_vs_phi_pTl%i",pTl);
  hndeterm_vs_phi[pTl]=new TH2D(name,"dete jet pt(for RM) vs phi;#phi;p_{T,jet}^{det}(GeV/c)",200,0,8,nptbins, ptminbin, ptmaxbin);

  //eta efficiency
  name=Form("hnture_all_eta_pTl%i",pTl);
  hntrue_all_eta[pTl]=new TH1D(name,"n true generated eta (for efficiency);#eta;entries",100,-1,1);
  name=Form("hnture_rm_eta_pTl%i",pTl);
  hntrue_rm_eta[pTl]=new TH1D(name,"n true generated eta (for RM);#eta;entries",100,-1,1);

  //phi efficiency
  name=Form("hnture_all_phi_pTl%i",pTl);
  hntrue_all_phi[pTl]=new TH1D(name,"n true generated phi (for efficiency);#phi;entries",1000,0,8);
  name=Form("hnture_rm_phi_pTl%i",pTl);
  hntrue_rm_phi[pTl]=new TH1D(name,"n true generated phi (for RM);#phi;entries",1000,0,8);

  
}

cout<<"reading eff"<<endl;
char name[200];
if(kCen){
	TFile *f_read_3deff=new TFile("/star/u/yanghe/Isobar_200_jet/toymodel/Production/3d_efficiency_Central.root","OPEN");
	sprintf(name,"h3d_effi");
	h3dEff = (TH3D*)f_read_3deff->Get(name);
	int testn= h3dEff ->GetEntries();
        cout<<"test n: "<<testn<<endl;
}
else{
	TFile *f_read_3deff=new TFile("/star/u/yanghe/Isobar_200_jet/toymodel/Production/3d_efficiency_peripheral.root","OPEN");
	sprintf(name,"h3d_effi");
	h3dEff = (TH3D*)f_read_3deff->Get(name);
	int testn= h3dEff ->GetEntries();
        cout<<"test n: "<<testn<<endl;
...
cout<<"reading eff sucessfully"<<endl;
// SETTING RANDOM SEED
  TDatime dt;
...
//_________new part
//_________readin 3d eff____________
/*
void ThrmPythiaEmb::read3dEff(){
if(kCen){
	TFile* f_read_3deff=new TFile("3d_efficiency_Central.root");
	sprintf(name,"h3d_effi");
	h3dEff = (TH3D*)f_read_3deff->Get(name);
}
else{
...
cout<<"Memory used: "<<memstat.Used()<<endl;
  ...
  //for(Int_t iemb = 0; iemb < nEmb-1; iemb++)
	...
	//create pythia jet
	Double_t PtJet=MakeCone(fpythia, detearr, pararr, pT, jetEta, jetPhi, kCharged, kEfficorr,kpTsmear,usefrag, true, hTrackEff,hparticle,h3dEff); 
	   ...
	   //FILLING EFFICIENCY HISTOGRAMS
		for(Int_t pTl=0; pTl<npTlead; pTl++){
			if(pTleading_par>pTl /*&& area_par>fAreaCut*/)
			{
				...
				hntrue_all_eta[pTl]->Fill(eta_par);
				hntrue_all_phi[pTl]->Fill(phi_par);
			}
	   ...
	   if(pTleading_dete0>pTl)
		{
			hJetEff2D[pTl]->Fill(pT_dete0, pT_par);//FILLING EFFICIENCY HISTOGRAM
			  ...
			  hntrue_rm_eta[pTl]->Fill(eta_par);
			  hntrue_rm_phi[pTl]->Fill(phi_par);
			  ...
			  hntruerm_vs_phi[pTl]->Fill(phi_par,pT_par);
			  hndeterm_vs_phi[pTl]->Fill(phi_dete0,pT_dete0);
...
for(int pTl=0; pTl<npTlead; pTl++){
  ...
  for(int pTl=0; pTl<npTlead; pTl++){
	hntrue_rm_eta[pTl]->Divide(hntrue_all_eta[pTl]);
	hntrue_rm_eta[pTl]->Scale(scaler);
	hntrue_rm_eta[pTl]->Write(Form("heta_effi_pTl%i",pTl));

	hntrue_rm_phi[pTl]->Divide(hntrue_all_phi[pTl]);
	hntrue_rm_phi[pTl]->Scale(scaler);
	hntrue_rm_phi[pTl]->Write(Form("hphi_effi_pTl%i",pTl));	

	hntrue_rm[pTl]->Divide(hntrue_all[pTl]);
	hntrue_rm[pTl]->Scale(scaler);
	hntrue_rm[pTl]->Write(Form("hpt_effi_pTl%i",pTl));	
/*	
	hJetEff1D[pTl]=(TH1D*) hJetEff2D[pTl]->ProjectionY(Form("hepsilon_pTl%i",pTl), 1, hJetEff2D[pTl]->GetXaxis()->GetNbins());
	hJetEff1D[pTl]->SetTitle("jet reconstruction efficiency");
	hRM_projectionY[pTl]=(TH1D*) hresponse[pTl]->ProjectionY(Form("hresponseY_pTl%i",pTl), 1, hresponse[pTl]->GetXaxis()->GetNbins());
