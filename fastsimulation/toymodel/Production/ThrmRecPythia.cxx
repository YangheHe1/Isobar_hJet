...
//---------------------------------------------
Double_t MakeCone(TPythia6 *pythia, TClonesArray *detearr, TClonesArray *pararr,Double_t pT, Double_t jetEta, Double_t jetPhi, Bool_t charged, Bool_t effi, Bool_t pTsmear, TString frag, bool fillEffHisto, TH1D* hTrackEff,TH1D* hparticle,TH3D* h3dEff)
{
  ...
  Int_t do3deff = atoi(gSystem->Getenv("Do3D_Eff"));
		   ...
		   if(effi)
			{
			   ...
			   if(do3deff){
				double pt_3d=pTpart;
				if(pt_3d>4.9) pt_3d=4.9;
				int xbin = h3dEff->GetXaxis()->FindBin(pt_3d);
        			int ybin = h3dEff->GetYaxis()->FindBin(eta);
        			int zbin = h3dEff->GetZaxis()->FindBin(phi);
        			int bin = h3dEff->GetBin(xbin,ybin,zbin);
        			epsilon = h3dEff->GetBinContent(bin);
				//cout<<xbin<<"  "<<ybin<<"  "<<zbin<<"  "<<bin<<"  "<<epsilon<<endl; 
				//epsilon=efficiency3d(pTpart, eta, phi, kCentral,h3d_effi); 
				}
			   else{ epsilon=efficiencyAlex(pTpart, kCentral,ratio, eff_increment);} //efficiency function 
