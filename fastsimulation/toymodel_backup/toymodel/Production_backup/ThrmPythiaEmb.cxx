...
for(Int_t pTl=0; pTl<npTlead; pTl++)
{
  ...
  hresponse[pTl]=new TH2D(name, "ResponseMatrix;p_{T}^{meas};p_{T}^{true};entries", nptbins, ptminbin, ptmaxbin, nptbins, ptminbin, ptmaxbin);
  ...
  hntrue_all[pTl]=new TH1D(name, "n true generated (for efficiency);p_{T}^{true};entries", nptbins, ptminbin, ptmaxbin);
  name=Form("hntrue_rm_pTl%i",pTl);
  hntrue_rm[pTl]=new TH1D(name, "n true generated (for RM);p_{T}^{true};entries", nptbins, ptminbin, ptmaxbin);
...
cout<<"Memory used: "<<memstat.Used()<<endl;
  ...
  //for(Int_t iemb = 0; iemb < nEmb-1; iemb++)
	...
	//create pythia jet
	Double_t PtJet=MakeCone(fpythia, detearr, pararr, pT, jetEta, jetPhi, kCharged, kEfficorr,kpTsmear,usefrag, true, hTrackEff); 
      ...
      //vector<PseudoJet> constituents = sorted_by_pt(akt_data2.clust_seq->constituents(jets2[djet]));
	   ...
	   Double_t pTleading_dete = constituents2[0].perp();
		...
		for(Int_t pTl=0; pTl<npTlead; pTl++){
  	   ...
  	   //JET MATCHING I - find closest detector level jet
