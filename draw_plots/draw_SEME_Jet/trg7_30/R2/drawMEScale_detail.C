...
*/    
    ...
    gr[0]->SetFillStyle(3002);;
            ...
            if(_form==0) hhtem[0] -> GetXaxis()->SetRangeUser(-5,10);
	        ...
	        tx0=0.50, ty0=0.69;
	        myTextF(tx0,ty0,"h^{#pm}+jet,7<P_{T}^{tirg}<30GeV/c",tsize*1.,1,12);
...
//            leg->AddEntry(gr[1],"Fill region range2","f");
    ...
    if(_form==0) hhtem[3] -> GetXaxis()->SetRangeUser(-5,10);
    ...
    TLine *t1;
    if(_form==0) t1= new TLine(-5,1,10,1);
    if(_form==1) t1= new TLine(-2,1,10,1);
    t1->SetLineStyle(2);
	t1->Draw("same");  
