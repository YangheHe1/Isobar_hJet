...
void readin(){
    ...
    for(int ibin=1;ibin<=binmax;ibin++){
	    ...
	    hRatioP = (TH1D*) hRho_SEP->Clone();
    Int_t binmax=hRho_SEP->FindLastBinAbove(0,1);
    for(int ibin=1;ibin<=binmax;ibin++){
        double y = hRho_SEP->GetBinContent(ibin);
        double yref = hRho_MEP->GetBinContent(ibin);
        if(y==0||yref==0)
                        {hRatioP->SetBinContent(ibin,0);
                                continue;}
        double binerr = hRho_SEP->GetBinError(ibin);
        double binerref =hRho_MEP->GetBinError(ibin);
        hRatioP->SetBinContent(ibin,y/yref);
        double err = (binerr/yref)-(y*binerref/(yref*yref));
        hRatioP->SetBinError(ibin,err);
...
void drawRhoCompare(){
    ...
    hhtem[5] -> SetLineStyle(8);
            ...
            tx0=0.3;ty0=0.72;
            myTextF(tx0,ty0,"k_{T} algorithm,R=0.4",tsize*1.1,1,12);
