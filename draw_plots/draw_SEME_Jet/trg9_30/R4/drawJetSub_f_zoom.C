...
void readin(){
    ...
    //===============================  read in =====================================
    sprintf(name,"jet_Pythia6_R4.root");
    cout<<name<<endl;
    fin = TFile::Open(name);
...
*/    
    ...
    gr[0]->SetFillStyle(3002);;
            ...
            if(_form==0) hhtem[0] -> GetXaxis()->SetRangeUser(-10,30);
            ...
            myTextF(tx0,ty0,"Anti-k_{T}, R=0.4",tsize*1.,1,12);

	        tx0=0.50, ty0=0.69;
	        myTextF(tx0,ty0,"h^{#pm}+jet,9<P_{T}^{trg}<30GeV/c",tsize*1.,1,12);
...
//            leg->AddEntry(gr[1],"Fill region range2","f");
    ...
    if(_form==0) hhtem[3] -> GetXaxis()->SetRangeUser(-10,30);
    ...
    TLine *t1;
    if(_form==0) t1= new TLine(-10,1,30,1);
...
void drawSESubME(){
    ...
    hhtem[2]->SetLineColor(kGreen+2);
            ...
            tx0=0.50;ty0=0.76;
            //sprintf(name,"centrality 0-20%");
            myTextF(tx0,ty0,"Anti-k_{T}, R=0.4",tsize*0.8,1,12);
